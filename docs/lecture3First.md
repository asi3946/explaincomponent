...
### UserCardコンポーネント
![w:450](../public/UserCardLec1.png)

</div>
<div>

### Page全体
![w:450](../public/PageLec1.png)
...
外部にUserCard.module.cssというファイル গঠনし、枠線や余白のクラスを定義していきます。
...
### CSSを適応したユーザカード
![w:450](../public/UserCardLec2v1.png)

</div>
<div>

### CSSの配置
![w:450](../public/UserCardLec2v2.png)
...
![bg right:60% contain](../public/PageLec4.png)
...
![bg right:60% contain](../public/CardListLec5.png)
###### カードリストコンポーネント
...
![bg right:60% contain](../public/PageLec6.png)
...
--- 
# 補足資料3：見出しタグ（h1, h2）と文字サイズ

見出しタグ（`h1`, `h2`, `h3`...）は、文字の大きさではなく**文書の階層構造**を示すためのものです。

- `h1`：ページ全体の大見出し（基本的に1ページに1つ）
- `h2`：h1の中にある中見出し
- `h3`：h2の中にある小見出し

文字を大きくしたい場合は、構造を無視して `h1` を使うのではなく、正しい見出しタグを選択した上で、CSSの `font-size` で見た目を調整するのが正しい設計です。


---

# 補足資料3：見出しタグ（h1, h2）と文字サイズ
今回の講座では、UserCard.tsxにおいて\<h2>タグを使用していましたが、
これは、ヘッダーのタイトルに\<h1>タグが使用されると予想されるためです。
と言っても、今回の実装のように作成するページを意識しながら見出しレベル(タグの番号)を決定するのはコンポーネントの考え方的ではないです。
そのため、見出しレベルを変数として実装する等の対処があります。
...
Reactコンポーネントを作る際、関数を宣言する方法は主に2種類あります。
...
どちらを使っても動作はほぼ同じです。モダンな開発現場では①のアロー関数が使われることが多いらしいので、本講座でもアロー関数を採用しています。ほぼ同じでない部分を知りたい人は参考資料を参考してください。難しいので、私も理解できていません。
参考資料:
- サバイバルTypeScript 従来の関数とアロー関数の違い
https://typescriptbook.jp/reference/functions/function-expression-vs-arrow-functions
---

# 補足資料5：return後の括弧 () の省略

JSXを `return` で返すとき、括弧 `()` を書く場合と省略する場合があります。

- **複数行になる場合**
  JavaScriptが途中で処理の終わりだと勘違いするのを防ぐため `()` で囲みます。
  ```tsx
  return (
    <div>
      ...
...
コンポーネントがデータ（Props）を受け取る書き方には、主に2つのパターンがあります。
本講座ではコードがすっきりして読みやすくなるパターン1を採用しています。

**パターン1：中身を直接取り出して受け取る（分割代入）**
```tsx
const UserCard = ({ name, comment }: UserCardProps) => {
  return (
    <div>
...
```
パターン2については「動画2:ルーティングの考え方」で解説しています。

---

# 補足資料7：おすすめ動画
【永久保存版】世界一簡単なReact講座・JavaScript初心者は必見！クイズアプリを作って学ぼう
https://youtu.be/TgU-FT2WdS4?si=aVdjG8iSV4lYWj26
